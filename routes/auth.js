...
//     const overallStats = {
//       totalTestsTaken: progress.length,
//       averageScore: progress.length 
//         ? progress.reduce((sum, p) => sum + (p.score / p.totalMarks * 100), 0) / progress.length 
//         : 0,
//       bestScore: progress.length 
//         ? Math.max(...progress.map(p => p.score / p.totalMarks * 100)) 
//         : 0
//     };

//     res.json({
//       overallStats,
//       testProgress: progress.map(p => ({
//         testType: p.testType,
//         testId: p.testId,
//         score: p.score,
...
// // Save Progress Route
// router.post('/progress/save', authMiddleware, async (req, res) => {
//   const { testType, testId, score, totalMarks, accuracy, timeSpent } = req.body;
...
//       score,
//       totalMarks,
//       accuracy: accuracy || (score / totalMarks * 100),
//       dateTaken: new Date(),
//       details: { timeSpent: timeSpent || 0 }
//     };
...
//     res.status(201).json({ message: 'Progress saved successfully', progress: progressEntry });
//   } catch (err) {
//     console.error('Progress Save Error:', err);
//     res.status(500).json({ message: 'Server error', error: err.message });
//   }
